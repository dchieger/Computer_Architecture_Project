...
def generate_example_traces():
    ...
    generate_trace_file(
        "trace_files/trace1_sequential.trc", num_entries=1000, sequential_ratio=0.9
    )

    # Random memory access pattern (data structure traversal)
    generate_trace_file(
        "trace_files/trace2_random.trc", num_entries=1000, sequential_ratio=0.3
    )

    # Mixed memory access pattern
    generate_trace_file(
        "trace_files/trace3_mixed.trc", num_entries=1000, sequential_ratio=0.6
    )
